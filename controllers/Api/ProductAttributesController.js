...
const fs = require("fs");
exports.selectProductAttributesByFilter = async (req, res, next) => {
    var params = { limit: 10, page: 1, constrains: { name: "" } };
    const result = await filter.filter("product_attributes", params);
    if (result) {
...
exports.search = async (req, res, next) => {
    ...
    await conn.product_attributes
        ...
        .then(async function (assets) {
            var count = conn.product_attributes.findAll();
            res.status(200).json({
                status: true,
                data: assets,
                tot: count.length,
...
exports.getProductAttributes = async (req, res, next) => {
    try {
        const result = await conn.product_attributes.findAll();
        res.status(200).json({ status: true, data: result });
        // res.status(500).json({
        //   status: false,
...
exports.createProductAttributes = async (req, res, next) => {
    try {
        const result = await conn.product_attributes.create(req.body);
        res.status(200).json({ status: true, data: result });
    } catch (e) {
        console.log(e);
        res.status(500).json({
...
exports.paginate = async (req, res, next) => {
    try {
        ...
        const count = await conn.product_attributes.findAll();
...
//@route  GET
//@access Public
exports.getProductAttributesById = async (req, res, next) => {
    try {
        const result = await conn.product_attributes.findOne({
...
exports.updateProductAttributes = async (req, res, next) => {
    try {
        const attribute = await conn.product_attributes.findOne({
            where: {
                id: req.params.id,
            },
        });
        ...
        await conn.product_attributes.update(req.body, {
            where: { id: req.params.id },
        });
...
//@route  Delete
//@access Public
exports.deleteProductAttributes = async (req, res, next) => {
    try {
        const attribute = await conn.product_attributes.findOne({
            where: {
                id: req.params.id,
        ...
        await conn.product_attributes.destroy({
            where: { id: req.params.id },
        });
        res.status(200).json({
            status: true,
