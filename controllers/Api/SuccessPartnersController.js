...
const fs = require("fs");
exports.selectSuccessPartnersByFilter = async (req, res, next) => {
    var params = { limit: 10, page: 1, constrains: { name: "" } };
    const result = await filter.filter("success_partners", params);
    if (result) {
...
exports.search = async (req, res, next) => {
    ...
    await conn.success_partners
        ...
        .then(async function (assets) {
            var count = conn.success_partners.findAll();
            res.status(200).json({
                status: true,
                data: assets,
                tot: count.length,
...
//@route  GET
//@access Public
exports.getSuccessPartners = async (req, res, next) => {
    try {
        const result = await conn.success_partners.findAll();
        res.status(200).json({ status: true, data: result });
        // res.status(500).json({
        //   status: false,
...
exports.createSuccessPartners = async (req, res, next) => {
    try {
        const result = await conn.success_partners.create(req.body);
        res.status(200).json({ status: true, data: result });
    } catch (e) {
        console.log(e);
        res.status(500).json({
...
exports.paginate = async (req, res, next) => {
    try {
        ...
        const count = await conn.success_partners.findAll();
...
//@route  GET
//@access Public
exports.getSuccessPartnersById = async (req, res, next) => {
    try {
        const result = await conn.success_partners.findOne({
...
//@route  Put
//@access Public
exports.updateSuccessPartners = async (req, res, next) => {
    try {
        const category = await conn.success_partners.findOne({
            where: {
                id: req.params.id,
            },
        });
        ...
        await conn.success_partners.update(req.body, {
            where: { id: req.params.id },
        });
...
//@route  Delete
//@access Public
exports.deleteSuccessPartners = async (req, res, next) => {
    try {
        const category = await conn.success_partners.findOne({
            where: {
                id: req.params.id,
        ...
        await conn.success_partners.destroy({
            where: { id: req.params.id },
        });
        res.status(200).json({
            status: true,
