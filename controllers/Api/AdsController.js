...
const fs = require("fs");
exports.selectAdsByFilter = async (req, res, next) => {
    var params = { limit: 10, page: 1, constrains: { name: "" } };
    const result = await filter.filter("ads", params);
    if (result) {
...
exports.getAds = async (req, res, next) => {
    try {
        const result = await conn.ads.findAll();
        res.status(200).json({ status: true, data: result });
        // res.status(500).json({
        //   status: false,
...
exports.createAds = async (req, res, next) => {
    try {
        const result = await conn.ads.create(req.body);
        res.status(200).json({ status: true, data: result });
    } catch (e) {
        console.log(e);
        res.status(500).json({
...
exports.paginate = async (req, res, next) => {
    try {
        ...
        const count = await conn.ads.findAll();
...
//@route  GET
//@access Public
exports.getAdsById = async (req, res, next) => {
    try {
        const result = await conn.ads.findOne({
...
exports.updateAds = async (req, res, next) => {
    try {
        const category = await conn.ads.findOne({
            where: {
                id: req.params.id,
            },
        });
        ...
        await conn.ads.update(req.body, {
            where: { id: req.params.id },
        });
