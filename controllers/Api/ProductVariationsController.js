...
const fs = require("fs");
exports.selectProductVariationsByFilter = async (req, res, next) => {
    var params = { limit: 10, page: 1, constrains: { name: "" } };
    const result = await filter.filter("product_variations", params);
    if (result) {
...
exports.search = async (req, res, next) => {
    ...
    await conn.product_variations
        ...
        .then(async function (assets) {
            var count = conn.product_variations.findAll();
            res.status(200).json({
                status: true,
                data: assets,
                tot: count.length,
...
exports.getProductVariations = async (req, res, next) => {
    try {
        const result = await conn.product_variations.findAll();
        res.status(200).json({ status: true, data: result });
        // res.status(500).json({
        //   status: false,
...
exports.createProductVariations = async (req, res, next) => {
    try {
        const result = await conn.product_variations.create(req.body);
        res.status(200).json({ status: true, data: result });
    } catch (e) {
        console.log(e);
        res.status(500).json({
...
exports.paginate = async (req, res, next) => {
    try {
        ...
        const count = await conn.product_variations.findAll();
...
//@route  GET
//@access Public
exports.getProductVariationsById = async (req, res, next) => {
    try {
        const result = await conn.product_variations.findOne({
...
//@route  Put
//@access Public
exports.updateProductVariations = async (req, res, next) => {
    try {
        await conn.product_variations.update(req.body, {
            where: { id: req.params.id },
        });
...
//@route  Delete
//@access Public
exports.deleteProductVariations = async (req, res, next) => {
    try {
        await conn.product_variations.destroy({
            where: { id: req.params.id },
        });
        res.status(200).json({
            status: true,
