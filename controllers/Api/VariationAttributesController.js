...
const fs = require("fs");
exports.selectVariationAttributesByFilter = async (req, res, next) => {
    var params = { limit: 10, page: 1, constrains: { name: "" } };
    const result = await filter.filter("variation_attributes", params);
    if (result) {
...
exports.search = async (req, res, next) => {
    ...
    await conn.variation_attributes
        .findAll({
        ...
        .then(async function (assets) {
            var count = conn.variation_attributes.findAll();
            res.status(200).json({
                status: true,
                data: assets,
                tot: count.length,
...
//@route  GET
//@access Public
exports.getVariationAttributes = async (req, res, next) => {
    try {
        const result = await conn.variation_attributes.findAll();
        res.status(200).json({ status: true, data: result });
        // res.status(500).json({
        //   status: false,
...
exports.createVariationAttributes = async (req, res, next) => {
    try {
        const result = await conn.variation_attributes.create(req.body);
        res.status(200).json({ status: true, data: result });
    } catch (e) {
        console.log(e);
        res.status(500).json({
...
exports.paginate = async (req, res, next) => {
    try {
        ...
        const count = await conn.variation_attributes.findAll();
...
//@route  GET
//@access Public
exports.getVariationAttributesById = async (req, res, next) => {
    try {
        const result = await conn.variation_attributes.findOne({
...
//@route  Put
//@access Public
exports.updateVariationAttributes = async (req, res, next) => {
    try {
        const category = await conn.variation_attributes.findOne({
            where: {
                id: req.params.id,
            },
        });
        ...
        await conn.variation_attributes.update(req.body, {
            where: { id: req.params.id },
        });
...
//@route  Delete
//@access Public
exports.deleteVariationAttributes = async (req, res, next) => {
    try {
        const category = await conn.variation_attributes.findOne({
            where: {
                id: req.params.id,
        ...
        await conn.variation_attributes.destroy({
            where: { id: req.params.id },
        });
        res.status(200).json({
            status: true,
