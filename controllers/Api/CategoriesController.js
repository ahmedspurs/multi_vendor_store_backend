...
const fs = require("fs");
exports.selectCategoriesByFilter = async (req, res, next) => {
  var params = { limit: 10, page: 1, constrains: { name: "" } };
  const result = await filter.filter("categories", params);
  if (result) {
...
exports.search = async (req, res, next) => {
  ...
  await conn.categories
    ...
    .then(async function (assets) {
      var count = conn.categories.findAll();
...
exports.getCategories = async (req, res, next) => {
  try {
    const result = await conn.categories.findAll();
    res.status(200).json({ status: true, data: result });
    // res.status(500).json({
    //   status: false,
...
exports.createCategories = async (req, res, next) => {
  try {
    const result = await conn.categories.create(req.body);
    res.status(200).json({ status: true, data: result });
  } catch (e) {
    console.log(e);
    res.status(500).json({
...
exports.paginate = async (req, res, next) => {
  try {
    ...
    const count = await conn.categories.findAll();
...
exports.getCategoriesById = async (req, res, next) => {
  try {
    const result = await conn.categories.findOne({
...
exports.updateCategories = async (req, res, next) => {
  try {
    const category = await conn.categories.findOne({
      where: {
        id: req.params.id,
      },
    });
    ...
    await conn.categories.update(req.body, {
      where: { id: req.params.id },
    });
