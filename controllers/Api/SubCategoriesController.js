...
const fs = require("fs");
exports.selectSubCategoriesByFilter = async (req, res, next) => {
    var params = { limit: 10, page: 1, constrains: { name: "" } };
    const result = await filter.filter("SubCategories", params);
    if (result) {
...
exports.search = async (req, res, next) => {
    ...
    await conn.sub_categories
        ...
        .then(async function (assets) {
            var count = conn.sub_categories.findAll();
            res.status(200).json({
                status: true,
                data: assets,
                tot: count.length,
...
exports.getSubCategories = async (req, res, next) => {
    try {
        const result = await conn.sub_categories.findAll();
        res.status(200).json({ status: true, data: result });
        // res.status(500).json({
        //   status: false,
...
exports.createSubCategories = async (req, res, next) => {
    try {
        const result = await conn.sub_categories.create(req.body);
        res.status(200).json({ status: true, data: result });
    } catch (e) {
        console.log(e);
        res.status(500).json({
...
exports.paginate = async (req, res, next) => {
    try {
        ...
        const count = await conn.sub_categories.findAll();
...
exports.getSubCategoriesById = async (req, res, next) => {
    try {
        const result = await conn.sub_categories.findOne({
...
exports.updateSubCategories = async (req, res, next) => {
    try {
        const category = await conn.sub_categories.findOne({
            where: {
                id: req.params.id,
            },
        });
        ...
        await conn.sub_categories.update(req.body, {
            where: { id: req.params.id },
        });
...
exports.deleteSubCategories = async (req, res, next) => {
    try {
        ...
        await conn.sub_categories.destroy({
            where: { id: req.params.id },
        });
        res.status(200).json({
            status: true,
