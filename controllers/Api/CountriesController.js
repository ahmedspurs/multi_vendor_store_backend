...
const fs = require("fs");
exports.selectCountriesByFilter = async (req, res, next) => {
    var params = { limit: 10, page: 1, constrains: { name: "" } };
    const result = await filter.filter("countries", params);
    if (result) {
...
exports.search = async (req, res, next) => {
    ...
    await conn.countries
        ...
        .then(async function (assets) {
            var count = conn.countries.findAll();
            res.status(200).json({
                status: true,
                data: assets,
                tot: count.length,
...
exports.getCountries = async (req, res, next) => {
    try {
        const result = await conn.countries.findAll({
            include: ["cities"],
        });
        res.status(200).json({ status: true, data: result });
        // res.status(500).json({
...
exports.createCountries = async (req, res, next) => {
    try {
        const result = await conn.countries.create(req.body);
        res.status(200).json({ status: true, data: result });
    } catch (e) {
        console.log(e);
        res.status(500).json({
...
exports.paginate = async (req, res, next) => {
    try {
        ...
        const count = await conn.countries.findAll();
...
exports.getCountriesById = async (req, res, next) => {
    try {
        const result = await conn.countries.findOne({
...
exports.updateCountries = async (req, res, next) => {
    try {
        const category = await conn.countries.findOne({
            where: {
                id: req.params.id,
            },
        });
        ...
        await conn.countries.update(req.body, {
            where: { id: req.params.id },
        });
