...
exports.selectProductImagesByFilter = async (req, res, next) => {
  var params = { limit: 10, page: 1, constrains: { name: "" } };
  const result = await filter.filter("product_images", params);
  if (result) {
...
exports.search = async (req, res, next) => {
  ...
  await conn.product_images
    ...
    .then(async function (assets) {
      var count = conn.product_images.findAll();
...
exports.getProductImages = async (req, res, next) => {
  try {
    const result = await conn.product_images.findAll();
    res.status(200).json({ status: true, data: result });
  } catch (e) {
    console.log(e);
...
exports.createProductImages = async (req, res, next) => {
  try {
    const result = await conn.product_images.create(req.body);
    res.status(200).json({ status: true, data: result });
  } catch (e) {
    console.log(e);
    res.status(500).json({ status: false, msg: `حدث خطأ ما في السيرفر` });
...
exports.paginate = async (req, res, next) => {
  try {
    ...
    var result = await conn.product_images.findAll({
      order: [["id", "DESC"]],
      include: [],
      offset: offset,
      limit: req.body.limit,
    ...
    var count = await conn.product_images.findAll();
...
//@route  GET
//@access Public
exports.getProductImagesById = async (req, res, next) => {
  try {
    const result = await conn.product_images.findOne({
...
exports.updateProductImages = async (req, res, next) => {
  try {
    await conn.product_images.update(req.body, {
      where: { id: req.params.id },
    });
...
exports.deleteProductImages = async (req, res, next) => {
  try {
    await conn.product_images.destroy({ where: { id: req.params.id } });
