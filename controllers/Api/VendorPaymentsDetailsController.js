...
const fs = require("fs");
exports.selectVendorPaymentsDetailsByFilter = async (req, res, next) => {
    var params = { limit: 10, page: 1, constrains: { name: "" } };
    const result = await filter.filter("vendor_payments_details", params);
    if (result) {
...
exports.search = async (req, res, next) => {
    ...
    await conn.vendor_payments_details
        .findAll({
        ...
        .then(async function (assets) {
            var count = conn.vendor_payments_details.findAll();
            res.status(200).json({
                status: true,
                data: assets,
                tot: count.length,
...
//@route  GET
//@access Public
exports.getVendorPaymentsDetails = async (req, res, next) => {
    try {
        const result = await conn.vendor_payments_details.findAll();
        res.status(200).json({ status: true, data: result });
        // res.status(500).json({
        //   status: false,
...
exports.createVendorPaymentsDetails = async (req, res, next) => {
    try {
        const result = await conn.vendor_payments_details.create(req.body);
        res.status(200).json({ status: true, data: result });
    } catch (e) {
        console.log(e);
        res.status(500).json({
...
exports.paginate = async (req, res, next) => {
    try {
        ...
        const count = await conn.vendor_payments_details.findAll();
...
//@route  GET
//@access Public
exports.getVendorPaymentsDetailsById = async (req, res, next) => {
    try {
        const result = await conn.vendor_payments_details.findOne({
...
//@route  Put
//@access Public
exports.updateVendorPaymentsDetails = async (req, res, next) => {
    try {
        const category = await conn.vendor_payments_details.findOne({
            where: {
                id: req.params.id,
            },
        });
        ...
        await conn.vendor_payments_details.update(req.body, {
            where: { id: req.params.id },
        });
...
//@route  Delete
//@access Public
exports.deleteVendorPaymentsDetails = async (req, res, next) => {
    try {
        const category = await conn.vendor_payments_details.findOne({
            where: {
                id: req.params.id,
        ...
        await conn.vendor_payments_details.destroy({
            where: { id: req.params.id },
        });
        res.status(200).json({
            status: true,
