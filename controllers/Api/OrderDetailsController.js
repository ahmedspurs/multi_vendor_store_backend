...
exports.selectOrderDetailsByFilter = async (req, res, next) => {
  var params = { limit: 10, page: 1, constrains: { name: "" } };
  const result = await filter.filter("order_details", params);
  if (result) {
...
exports.search = async (req, res, next) => {
  ...
  await conn.order_details
    ...
    .then(async function (assets) {
      var count = conn.order_details.findAll();
...
exports.getOrderDetails = async (req, res, next) => {
  try {
    const result = await conn.order_details.findAll();
    res.status(200).json({ status: true, data: result });
  } catch (e) {
    console.log(e);
...
exports.createOrderDetails = async (req, res, next) => {
  try {
    const result = await conn.order_details.create(req.body);
    res.status(200).json({ status: true, data: result });
  } catch (e) {
    console.log(e);
    res.status(500).json({
...
exports.paginate = async (req, res, next) => {
  try {
    ...
    var result = await conn.order_details.findAll({
      order: [["id", "DESC"]],
      include: [],
      offset: offset,
      limit: req.body.limit,
    ...
    var count = await conn.order_details.findAll();
...
exports.getOrderDetailsById = async (req, res, next) => {
  try {
    const result = await conn.order_details.findOne({
...
exports.updateOrderDetails = async (req, res, next) => {
  try {
    await conn.order_details.update(req.body, {
      where: { id: req.params.id },
    });
...
exports.deleteOrderDetails = async (req, res, next) => {
  try {
    const isDeleted = await conn.order_details.destroy({
      where: { id: req.params.id },
    });
